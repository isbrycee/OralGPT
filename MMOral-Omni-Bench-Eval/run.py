...
def main():
    ...
    if args.config is not None:
        ...
        args.judge_args = cfg['judger']
        assert len(args.judge_args)==1, '--judger in the config.json should be a dict with only one key'
    else:
        ...
        for _, dataset_name in enumerate(args.data):
            ...
            try:
                ...
                if args.judge_args is not None:
                    args.judge = list(args.judge_args.keys())[0]
                    judge_kwargs = {**judge_kwargs, **args.judge_args[args.judge]}
