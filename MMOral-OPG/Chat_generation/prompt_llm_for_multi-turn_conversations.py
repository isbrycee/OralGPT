...
def batch_process_llm_requests(
    input_dir: str,
    output_dir: str,
    loc_query_field: str = "loc_caption",
    med_query_field: str = "med_report",
    ...
    for filename in tqdm(json_files):
        ...
        try:
            ...
            with open(input_path, 'r', encoding='utf-8') as f:
                ...
                # 检查是否包含查询字段
                if loc_query_field not in data:
                    raise ValueError(f"JSON文件 {filename} 缺少必需的 '{loc_query_field}' 字段")
                if med_query_field not in data:
                    raise ValueError(f"JSON文件 {filename} 缺少必需的 '{med_query_field}' 字段")
                
                query =  'The structured location caption is: \n{}\n\n' \
                         'The textual examination report is: \n{}' \
                         .format(data[loc_query_field], data[med_query_field])
                ...
                if "```json" in llm_response:
                    ...
                    try:
                        ...
                        data[response_field] = _dict['conversation']
                        print(_dict['conversation'])
                        # 保存到新路径
                        ...
                        print("解析失败，错误信息为:", e)
                        return batch_process_llm_requests(input_dir,output_dir,loc_query_field,med_query_field,response_field,retries-1)
...
if __name__ == "__main__":
    ...
    output_dir = "/home/jinghao/projects/x-ray-VLM/dataset/mmoral-json-v1/test_conv"
    
    # 运行批量处理
    batch_process_llm_requests(input_dir, output_dir, loc_query_field="loc_caption", \
                               med_query_field="med_report", response_field="conversations")