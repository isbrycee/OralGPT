...
image_folder = "./image/OralGPT-RGB-Classification-Dataset/gingivitis/Gingivitis"
output_json_path = "./dental_json/classification/Gingivitis/Gingivitis_qa.json"
...
request = """ 
Output format: 
<Caption> Describe the intraoral image in detail, including the tooth and/or arch location, any noticeable gingival color changes (e.g., erythema, redness), swelling, contour alterations, or bleeding signs. Focus on visual features strongly associated with Gingivitis. </Caption>

<Think> 
1. Explain the visible features in the image that match the known characteristics of Gingivitis.  
2. Recall the typical visual appearance and symptoms of Gingivitis from professional knowledge.  
3. Confirm that the observed image appearance matches the knowledge-based visual representation of Gingivitis.  
</Think> 
...
for filename in image_files:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 构建标准化结果
            entry = {
                "id": f"Gingivitis_diagnosis_{len(results) + 1}",  # 自动递增 id
                "image": os.path.join("OralGPT-RGB-Classification-Dataset/gingivitis/Gingivitis", filename),
                "question": fixed_question,
                "cot_answer": cot_result,
                "Modality": "Intraoral photograph",
                "Disease category": "Gingivitis"
            }
