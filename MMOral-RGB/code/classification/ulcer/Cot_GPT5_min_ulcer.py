...
image_folder = "./image/OralGPT-RGB-Classification-Dataset/ulcer/"
output_json_path = "./dental_json/classification/ulcer/ulcer_qa.json"
...
request = """ 
Output format: 
<Caption> Describe the intraoral image in detail, including the tooth and/or arch location, and the appearance of any mucosal lesions. Focus on ulcer-related features such as a localized mucosal break, oval or round shallow depression, white or yellow fibrinous base, erythematous/erythematous halo margins, and surrounding tissue changes. </Caption>

<Think> 
1. Explain the visible features in the image that match the known characteristics of oral ulcer.  
2. Recall the typical clinical appearance and symptoms of oral ulcer from professional knowledge.  
3. Confirm that the observed image appearance matches the knowledge-based visual representation of oral ulcer, and rule out alternative conditions.  
</Think> 
...
if os.path.exists(output_json_path):
    with open(output_json_path, "r", encoding="utf-8") as f:
        results = json.load(f)
    processed_images = {entry["image"] for entry in results}
else:
    results = []
...
for filename in image_files:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 构建标准化结果
            entry = {
                "id": f"ulcer_diagnosis_{len(results) + 1}",  # 自动递增 id
                "image": os.path.join("OralGPT-RGB-Classification-Dataset/ulcer", filename),
                "question": fixed_question,
                "cot_answer": cot_result,
                "Modality": "Intraoral photograph",
                "Disease category": "ulcer"
            }
