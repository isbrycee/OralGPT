...
image_folder = "./image/OralGPT-RGB-Classification-Dataset/calculus"
output_json_path = "./dental_json/classification/calculus/calculus_qa.json"
...
##不同分类request要修改
request = """ 
Output format: <Caption> Describe the intraoral image in detail, including the tooth and/or arch location, any noticeable color changes, and the morphology, texture, and distribution of any deposits. Incorporate visual features strongly associated with Calculus, or with other diseases/conditions you are highly confident are present. </Caption> <Think> 1. Explain the visible features in the image that match the known characteristics of Calculus. 2. Recall the typical visual appearance and symptoms of Calculus from professional knowledge. 3. Confirm that the observed image appearance matches the knowledge-based visual representation of Calculus. </Think> <Answer>Summarize the matched disease(s)/condition(s) from step 3 of the above <Think> pipeline into one concise, fluent, professional medical diagnosis statement that clearly specifies the oral disease(s)/condition(s) present in the image, without including any treatment recommendations or management advice.</Answer> 
"""
...
for filename in image_files:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 构建标准化结果
            entry = {
                "id": f"calculus_diagnosis_{len(results) + 1}",  # 自动递增 id
                "image": os.path.join("OralGPT-RGB-Classification-Dataset/calculus", filename),
                "question": fixed_question,
                "cot_answer": cot_result,
                "Modality": "Intraoral photograph",
                "Disease category": "calculus"
            }
