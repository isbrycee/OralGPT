...
image_folder = "./image/OralGPT-RGB-Classification-Dataset/defect_of_dentition"
output_json_path = "./dental_json/classification/defect_of_dentition/defect_of_dentition_qa.json"
...
request = """ 
Output format: 
<Caption> Describe the intraoral image in detail, including the tooth and/or arch location, occlusion relationship, and the presence of any defects in the dentition. Focus on features such as missing teeth, abnormal tooth shapes, malalignment, crowding, spacing, rotated or tipped teeth, hypodontia, supernumerary teeth, and any associated structural anomalies. </Caption>

<Think> 
1. Explain the visible features in the image that match the known characteristics of dentition defects.  
2. Recall the typical clinical appearance and types of dentition defects from professional dental knowledge.  
3. Confirm that the observed image appearance matches the knowledge-based visual representation of dentition defects, and differentiate from normal variations or other dental conditions.  
</Think> 

<Answer> Summarize the matched dental condition(s) from step 3 of the above <Think> pipeline into one concise, fluent, professional medical diagnosis statement that clearly specifies the type of dentition defect and affected teeth, without including any treatment recommendations or management advice. </Answer>
"""
...
for filename in image_files:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 构建标准化结果
            entry = {
                "id": f"defect_of_dentition_diagnosis_{len(results) + 1}",  # 自动递增 id
                "image": os.path.join("OralGPT-RGB-Classification-Dataset/defect_of_dentition", filename),
                "question": fixed_question,
                "cot_answer": cot_result,
                "Modality": "Intraoral photograph",
                "Disease category": "defect_of_dentition"
            }
